...
<div class="mx-auto max-w-3xl ">
  <div
    ...
    <div class="flex items-center justify-between flex-wrap py-1 px-3">
      <button class="w-0 flex-1 flex items-center" on:click={userconnlistaction}>
...
<div class="mx-auto max-w-3xl ">
  <div
    class="bg-black grid grid-flow-row text-white rounded-lg m-3 p-2 cursor-pointer focus:outline-none focus-within:bg-black noHL">
    <div class="flex items-center justify-between flex-wrap py-1 px-3">
      <button class="w-0 flex-1 flex items-center" on:click={manageUsersaction}>
    ...
    <div class="flex flex-col">
      {#if manageUsersOpen === true}
        {#each $userlist as eachuser (eachuser?.Username)}
          <Useredit username={eachuser?.Username} selected={typenotostring(eachuser?.UserType)}
                    timestring={new Date(eachuser?.CreatedAt)?.toLocaleString()} />
        {/each}
      {/if}
    </div>
  </div>
...
<div class="mx-auto max-w-3xl ">
  <div
    class="bg-black grid grid-flow-row text-white rounded-lg m-3 p-2 cursor-pointer focus:outline-none focus-within:bg-black noHL">
    <div class="flex items-center justify-between flex-wrap py-1 px-3">
      <button class="w-0 flex-1 flex items-center" on:click={useraddaction}>
        <p class="ml-3 font-medium  truncate">Add User</p>
      </button>

      <button type="button" class="-mr-1 flex p-2 rounded-md bg-neutral-800 focus:outline-none flex-shrink-0 mx-1"
              on:click={useraddaction}>
        {#if addUserOpen === true}
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
          </svg>
      ...
      {#if addUserOpen === true}
        <div class="mt-2">
          ...
          <input id="username" name="email" type="text" bind:value={newusername} required
                 class="bg-neutral-800 appearance-none rounded-md w-full px-3 py-2 border border-neutral-800 placeholder-neutral-500 text-neutral-200 focus:outline-none"
          ...
          <select
            class="bg-neutral-800 rounded-md w-full flex-grow px-3 py-2 border-none placeholder-neutral-500 text-neutral-200 focus:outline-none"
          ...
          <button type="button" on:click={addUser}
                  class="w-full my-2 py-2 px-4 border-none text-sm font-medium rounded-md text-white bg-indigo-900 outline-none focus:outline-none">
            Add User
          </button>
        </div>
      {/if}
