<script lang="ts">
  ...
  import { addToast, Send } from './core';
  ...
  let updateuser = () => {
    ...
    if (utchangeallowed === true) {
      if (selected !== oldselected) {
        ...
        addToast({ message: 'Same Usertype Selected', type: 'error' });
      }
    }
  };
</script>
  ...
  {#if usereditmode === true}
    <div class="flex mt-1">
      <input id="changepw" type="text" bind:value={newpw} required
             class=" bg-neutral-800 appearance-none rounded-md w-full flex-grow px-3 py-2 border-none placeholder-neutral-500 text-neutral-200  focus:outline-none sm:text-sm mx-1"
