...
<div class="bg-black text-neutral-200 rounded-lg m-3">
        ...
        <div class="text-neutral-400 flex justify-between text-sm font-medium tabular-nums">
          ...
          <div>
            {#if state === 'active' || state === 'inactive'}
              {#if seeding === true}(Seeding)
              {/if}{fileSize(bytesmissing == null ? 0 : bytesmissing)} R.
            {/if}
          </div>
        </div>
        {#if state !== 'removed'}
  ...
  <div
    class="bg-gradient-to-r from-black via-neutral-900 to-black text-white py-2 px-1 grid grid-flow-col grid-cols-3 justify-items-center mt-1 rounded-b-lg">
    ...
    {#if state === 'loading'}
      <button type="button" class="grid">
        <svg class="animate-spin h-6 w-6 mx-auto text-white" xmlns="http://www.w3.org/2000/svg" fill="none"
             viewBox="0 0 24 24">
