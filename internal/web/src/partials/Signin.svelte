<script lang="ts">
  ...
  import { addToast, Connect } from './core';
  ...
  function signIn() {
    if (exausername != '' && exausername != undefined && exausername != null) {
      if (exapassword != '' && exapassword != undefined && exapassword != null) {
        ...
        addToast({ message: 'Password Field Cannot be Empty', type: 'error' });
        return;
      }
    } else {
      addToast({ message: 'Username Field Cannot be Empty', type: 'error' });
      return;
    }
    return;
  }
...
<div class="mt-10  flex items-center justify-center px-4">
  <div class="max-w-md w-full ">
    ...
    <div class="mt-2">
      ...
      <input id="username" name="email" type="text" bind:value={exausername} required
             class="bg-neutral-800 my-2 appearance-none rounded-md w-full px-3 py-2 border border-neutral-800 placeholder-neutral-500 text-neutral-200 focus:outline-none"
      ...
      <button type="button" on:click={signIn}
              class="w-full my-2  py-2 px-4 border-none text-sm font-medium rounded-md text-white bg-blue-900  outline-none focus:outline-none">
        Sign in
      </button>
    </div>
  </div>
